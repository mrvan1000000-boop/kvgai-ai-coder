--- ai_coder/router.py ---
*(ВНИМАНИЕ: Это ПОЛНЫЙ файл. Не разделяй его, просто скопируй всё целиком и замени текущий файл)*
...
def call_model_with_fallback(messages, primary_model):
    ...
    if paid_fallback not in base: base.append(paid_fallback)
...
async def process_request(request, task, file, model, custom_model, user_id):
    ...
    save_message(uid, "user", msg["content"])
